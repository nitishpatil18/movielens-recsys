...
- **day 10 (week 2 day 4)** fixed the ood regression. `rebuild_ranker_dataset_v3.py` samples negatives from the serve-time candidate union (two-tower top-200 union sasrec top-200, masked by seen set) instead of v1's popularity-weighted random. retrained ranker_v3: auc_eval 0.853 (lower than v2's 0.915 by design; realistic negatives are harder). swept `RECSYS_SASREC_TOP_N` at {25, 50, 100, 200} on the live serve path; clear monotonic curve, n=50 wins overall. final result vs v1 on the same 1000 user sample (seed=42): recall@10 0.0506 vs 0.0491 (+3.1%), recall@20 0.0849 vs 0.0738 (+15%), recall@5 0.0274 vs 0.0308 (-11%), ndcg@10 0.1079 vs 0.1203 (-10%). modest top-10 win, solid top-20 win, top-5 and ranking quality slightly worse. set RECSYS_SASREC_TOP_N=50 as production default.
- **day 9 (week 2 day 3b)** ranker v2 retrained on the augmented dataset. lightgbm auc_eval 0.874 -> 0.915 (+4.7%). sasrec_score is the top feature by gain at ~8x tt_score. serving integration through `/recommend` regressed recall@10 vs v1 (0.0329 vs 0.0491). diagnosis: training negatives are popularity-weighted random items, but serve time pool is dominated by sasrec-retrieved candidates. ranker learned 'high sasrec_score -> positive' on a distribution where high-sasrec items were rare; at serve time they're the majority. textbook out-of-distribution failure. fix scheduled for day 10: rebuild ranker negatives from sasrec retrieval. feature-ordering bug also found and fixed during diagnosis (+17% on recall@10 alone, 0.0281 -> 0.0329, still short of v1).
- **day 8 (week 2 day 2)** sasrec integrated into serve.py behind two feature flags (`RECSYS_SASREC_ENABLED`, `RECSYS_RANKER_CKPT`). new helpers compute the user's sasrec hidden state once per request and reuse it for both retrieval and per-candidate scoring. warm-up forward pass at startup pays the mps kernel-compilation tax up front so the first request doesn't. m5 warm: total 9-10ms, sasrec stage 2-3ms, encode 0.7-1.4ms. flag off (default) keeps v1 behavior bit-identical with zero startup overhead. http-driven eval script `eval_union_protocol.py` measures /recommend against v1's exact 1000-user seed=42 sample so v1 (0.0491) and v1+sasrec runs are apples-to-apples on the deployed code path, not an offline approximation.
